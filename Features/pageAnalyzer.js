(function () {
  function collectTexts() {
    const selectors = "h1,h2,h3,h4,p,li,label,td,th,span,div";
    ...
    for (const node of nodes) {
      ...
      if (!raw || raw.length < 6 || raw.length > 260) continue;
  ...
  function pickQuestion(texts) {
    const explicit = texts.find((text) => /^(q(uestion)?\s*\d*[:.)-])/i.test(text) || text.endsWith("?"));
  ...
  function pickOptions(texts) {
    ...
    const options = texts.filter((text) => optionPattern.test(text));
